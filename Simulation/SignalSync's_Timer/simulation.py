...
parser.add_argument('--mode', type=str, default='adaptive',
                    help='Simulation mode: adaptive or fixed')
...
class Vehicle(pygame.sprite.Sprite):
    ...
    def move(self):
        # (movement logic unchanged — keeping your original)
...
def setTime():
    global L
    # your adaptive/fixed logic (unchanged)
    if simulation_mode == 'adaptive':
        A = sum(1 for lane_idx in range(3) for vehicle in vehicles['up'][lane_idx] if vehicle.crossed == 0)
        B = sum(1 for lane_idx in range(3) for vehicle in vehicles['right'][lane_idx] if vehicle.crossed == 0)
        C = sum(1 for lane_idx in range(3) for vehicle in vehicles['down'][lane_idx] if vehicle.crossed == 0)
        D = sum(1 for lane_idx in range(3) for vehicle in vehicles['left'][lane_idx] if vehicle.crossed == 0)
        T = A + B + C + D
        if T < 50: L = 50
        elif 50 <= T < 80: L = 60
        elif 80 <= T < 130: L = 80
        elif 130 <= T < 170: L = 100
        elif 170 <= T < 220: L = 120
        else: L = 140
        # proportional assignment skipped for brevity
    else:
    ...
    while True:
        ...
        for i in range(0,noOfSignals):
            if(i==currentGreen):
                ...
                eta = 0
                j = (currentGreen + 1) % noOfSignals
                while j != i:
                    eta += signals[j].green + signals[j].yellow
                    j = (j + 1) % noOfSignals
                eta += signals[i].red
                signals[i].signalText = str(eta)
        ...
        # ✅ Add SignalSync's LOGIC text
        logic_font = pygame.font.Font(None, 36)
        logic_surface = logic_font.render("SignalSync's LOGIC", True, black, white)
        screen.blit(logic_surface, (1100, 20))
